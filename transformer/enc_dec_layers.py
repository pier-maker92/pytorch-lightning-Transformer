...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, padding_mask=None):
        ...
        addnorm_output = self.add_norm_layers[0](multihead_output,x)
 
        # Followed by a fully connected layer
        feedforward_output = self.feed_forward(addnorm_output)
 
        # Followed by another Add & Norm layer
        return self.add_norm_layers[1](feedforward_output,addnorm_output)
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x_dec, x_enc, tgt_mask, src_padding_mask):
        ...
        # Followed by an Add & Norm layer
        addnorm_output = self.add_norm_layers[0](multihead_output,x_dec)

        if x_enc is not None:
            # Multi-head attention encoder - decoder
            multihead_output = self.multihead_attention(addnorm_output, x_enc, x_enc, src_padding_mask)

            # Followed by an Add & Norm layer
            addnorm_output = self.add_norm_layers[1](multihead_output,addnorm_output)

        # Followed by a fully connected layer
        feedforward_output = self.feed_forward(addnorm_output)
        
        # Followed by another Add & Norm layer
        return self.add_norm_layers[2](feedforward_output,addnorm_output)